...
fn get_latest_mew_hashes(limit: usize, before_mew_hash: Option<ActionHash>) -> ExternResult<Vec<ActionHash>> {
    let hashes = match before_mew_hash {
        Some(hash) => get_older_hashes(&MEW_TIME_INDEX_NAME.to_owned(), AnyLinkableHash::from(hash), limit)
            .map_err(|_| wasm_error!(WasmErrorInner::Guest("Failed to get older hashes".into())))?,
        None => get_latest_hashes(&MEW_TIME_INDEX_NAME.to_owned(), limit)
            .map_err(|_| wasm_error!(WasmErrorInner::Guest("Failed to get latest hashes".into())))?,
    };

    let action_hashes: Vec<ActionHash> = hashes
        .into_iter()
        .map(ActionHash::from)
        .collect();
