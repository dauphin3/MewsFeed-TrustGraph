use crate::{
    index_tree::IndexSegment, TimeIndexResult, TimeIndexingError, HAS_CHUNK_LEAVES, INDEX_DEPTH,
};
 ...
 */
pub fn read_all_hashes<I, IT: Copy + LinkTypeFilterExt, E>(
    index_name: &I,
    index_link_type: IT,
) -> TimeIndexResult<Vec<AnyLinkableHash>>
where
    I: AsRef<str>,
    ScopedLinkType: TryFrom<IT, Error = E>,
    WasmError: From<E>,
{
    let root_hash = get_root_hash(index_name, index_link_type.clone())?;

    match root_hash {
        None => Ok(vec![]),
        Some(hash) => Ok(collect_leaf_index_hashes(
            index_name,
            index_link_type,
            hash,
            (*INDEX_DEPTH).len() as isize,
        )?),
    }
}
...
///
fn collect_leaf_index_hashes<I, IT: Copy + LinkTypeFilterExt, E>(
    index_name: &I,
    index_link_type: IT,
    context_hash: AnyLinkableHash,
    context_depth: isize,
) -> TimeIndexResult<Vec<AnyLinkableHash>>
where
    I: AsRef<str>,
...
{
    let children =
        get_ordered_child_links_of_node(index_name, index_link_type, context_hash.clone())?;
    ...
    let (descendents, errors): (
        Vec<TimeIndexResult<Vec<AnyLinkableHash>>>,
        Vec<TimeIndexResult<Vec<AnyLinkableHash>>>,
    ) = children
        .iter()
        .map(|link| {
            collect_leaf_index_hashes(
                index_name,
                index_link_type,
 ...
 */
pub fn get_latest_hashes<I, IT: Copy + LinkTypeFilterExt, E>(
    index_name: &I,
    index_link_type: IT,
    limit: usize,
) -> TimeIndexResult<Vec<AnyLinkableHash>>
    ...
    WasmError: From<E>,
{
    ...
    let most_recent = get_latest_indexed_hash(index_name, index_link_type)?;

    match most_recent {
        None => Ok(vec![]),
        Some(latest) => {
            // load a page of links to entries immediately before the latest
            let mut earlier_page =
                get_older_hashes(index_name, index_link_type, latest.to_owned(), limit)?;
            ...
            Ok([vec![latest.to_owned()], earlier_page].concat())
        }
    }
}
 ...
 */
pub fn get_older_hashes<I, IT: Copy + LinkTypeFilterExt, E>(
    index_name: &I,
    index_link_type: IT,
    before_hash: AnyLinkableHash,
    limit: usize,
) -> TimeIndexResult<Vec<AnyLinkableHash>>
where
    I: AsRef<str>,
...
{
    Ok(
        get_ordered_links_before(index_name, index_link_type, before_hash, limit)?
            .iter()
            .map(|link| AnyLinkableHash::from(link.target.to_owned()))
            .collect(),
    )
...
///
fn get_ordered_links_before<I, IT: Copy + LinkTypeFilterExt, E>(
    index_name: &I,
    index_link_type: IT,
    hash: AnyLinkableHash,
    limit: usize,
) -> TimeIndexResult<Vec<Link>>
where
    I: AsRef<str>,
    ScopedLinkType: TryFrom<IT, Error = E>,
    WasmError: From<E>,
{
    ...
    let leaf_link = parents.first().ok_or(TimeIndexingError::NotIndexed(
        index_name.as_ref().to_string(),
        hash,
    ))?;
    ...
    let mut older_siblings = get_ordered_child_links_of_node_older_than(
        index_name,
        index_link_type,
        leaf_hash,
        this_timestamp,
    ...
    while older_siblings.len() < limit {
        ...
        match previous_leaf_link {
            ...
            Some(prev) => {
                ...
                let more_older_siblings = get_ordered_child_links_of_node_older_than(
                    index_name,
                    index_link_type,
                    AnyLinkableHash::from(prev.hash()?),
                    this_timestamp,
                )?;
...
///
fn get_ordered_child_links_of_node<I>(
    index_name: &I,
    link_type: impl LinkTypeFilterExt,
    leaf_hash: AnyLinkableHash,
) -> TimeIndexResult<Vec<Link>>
where
    I: AsRef<str>,
{
...
///
fn get_ordered_child_links_of_node_older_than<I>(
    index_name: &I,
    index_link_type: impl LinkTypeFilterExt,
    leaf_hash: AnyLinkableHash,
    this_timestamp: DateTime<Utc>,
) -> TimeIndexResult<Vec<Link>>
where
    I: AsRef<str>,
{
    // query children of parent node
    let siblings = get_ordered_child_links_of_node(index_name, index_link_type, leaf_hash)?;
...
/// :TODO: make this work
///
fn get_previous_leaf<I, IT: Copy + LinkTypeFilterExt, E>(
    index_name: &I,
    index_link_type: IT,
    from_timestamp: &DateTime<Utc>,
    try_depth: usize,
    starting_depth: usize,
) -> TimeIndexResult<Option<IndexSegment>>
where
    I: AsRef<str>,
    ScopedLinkType: TryFrom<IT, Error = E>,
    WasmError: From<E>,
{
    ...
    while this_depth >= starting_depth {
        let try_parent_hash = if this_depth >= INDEX_DEPTH.len() {
            ...
            Path::from(index_name.as_ref())
                .typed(index_link_type)?
                .path_entry_hash()?
        } else {
        ...
        let older_siblings = get_ordered_child_links_of_node_older_than(
            index_name,
            index_link_type,
            AnyLinkableHash::from(try_parent_hash),
        ...
        match older_siblings.first() {
            ...
            Some(link) => {
                ...
                return Ok(Some(IndexSegment::new(
                    from_timestamp,
                    INDEX_DEPTH.get(starting_depth).unwrap(),
                )));
            }
        }
    }
...
///
fn get_latest_indexed_hash<I, IT: Copy + LinkTypeFilterExt, E>(
    index_name: &I,
    index_link_type: IT,
) -> TimeIndexResult<Option<AnyLinkableHash>>
where
    I: AsRef<str>,
    ScopedLinkType: TryFrom<IT, Error = E>,
    WasmError: From<E>,
{
    match get_root_hash(index_name, index_link_type)? {
        None => Ok(None),
        // recurse into the tree, returning leafmost node
        Some(hash) => Ok(Some(get_newest_leafmost_hash(
            hash,
            index_name,
            index_link_type,
        )?)),
...
///
fn get_newest_leafmost_hash<I>(
    current_segment_hash: AnyLinkableHash,
    index_name: &I,
    index_link_type: impl LinkTypeFilterExt + Copy,
) -> TimeIndexResult<AnyLinkableHash>
where
    I: AsRef<str>,
{
    ...
    match children.last() {
        ...
        Some(l) => get_newest_leafmost_hash(
            AnyLinkableHash::from(l.target.to_owned()),
            index_name,
            index_link_type,
        ),
    }
